<header>
    <section class="logoHeader">
        <div>
            <img class="imageHeader" src="public/images/Mon joli cocon.png" alt="">   
        </div>
    </section>
    <section>
        <div class="titleLogo">
            <h3>Décoration d'intérieur</h3>
        </div>
    </section>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarSupportedContent">

            <a class="navbar-brand mx-auto" href="index.php?action=home">Home</a>

            <ul class="navbar-nav mr-auto"> 
                <li class="nav-item dropdown">
                    <a class="nav-link mx-auto" id="nav-link" href="index.php?action=creationChapter">Insérer photo</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link mx-auto" id="nav-link" href="index.php?action=creationChapter">Supprimer Photo</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link mx-auto" id="nav-link" href="index.php?action=creationAccount">Créer un compte admin</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link mx-auto" id="nav-link" href="index.php?action=deconnexion">Se déconnecter</a>
                </li>
            </ul>
        </div>
    </nav>
</header>